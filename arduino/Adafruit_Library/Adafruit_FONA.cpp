...
boolean Adafruit_FONA::postData(const char *request_type, const char *URL, String &serverResponse, const char *body, const char *token, uint32_t bodylen) {
  ...
  // *serverResponse = replybuffer;
  // strncpy(serverResponse, replybuffer, strlen(replybuffer));
  int i = 0;
  while (replybuffer[i] != '\0') {
    serverResponse += replybuffer[i];
    i++;
  }
...
uint8_t Adafruit_FONA::readline(uint16_t timeout, boolean multiline) {
  ...
  while (timeout--) {
    if (replyidx >= 499) { // change this number for a larger http res buffer
